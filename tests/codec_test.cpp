...
void test_envelope_refusals()
{
  const auto catalog = profiles();
  const auto snapshot = seal_source(
      source_origin("recipe.yml"), declaration(true), catalog);
...
void test_noncanonical_source_order()
{
  const auto catalog = profiles();
  const auto snapshot = seal_source(
      source_origin("recipe.yml"), declaration(false), catalog);
  ...
  const auto first = std::vector<std::uint8_t>(
      ...
      reordered.begin() + static_cast<std::ptrdiff_t>(layout.sources[0].second));
  const auto second = std::vector<std::uint8_t>(
      reordered.begin() + static_cast<std::ptrdiff_t>(layout.sources[1].first),
      reordered.begin() + static_cast<std::ptrdiff_t>(layout.sources[1].second));
  auto output = reordered.begin()
      + static_cast<std::ptrdiff_t>(layout.sources[0].first);
