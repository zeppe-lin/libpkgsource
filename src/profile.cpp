...
public:
  explicit catalog_builder(std::vector<profile_declaration> declarations)
  {
    std::sort(declarations.begin(), declarations.end(),
              [](const profile_declaration& lhs,
                 const profile_declaration& rhs) {
    ...
    for (profile_declaration& declaration : declarations) {
      ...
      records_.emplace(
          ...
          declaration_record{declaration.name(), declaration.provenance(),
                             std::move(members)});
    }
    ...
    for (const auto& entry : records_)
      for (const profile_member_declaration& member : entry.second.members)
        if (member.subject().kind() == requirement_subject_kind::profile
            && records_.find(member.subject().profile().name()) == records_.end())
  ...
  std::vector<sealed_profile> build()
  {
    ...
    for (const auto& entry : records_) {
      ...
      result.emplace_back(entry.second.name, computed.identity,
                          entry.second.provenance, entry.second.members,
                          computed.expansion);
    }
    return result;
  }
  ...
  const computed_profile& compute(const std::string& name)
  {
    ...
    for (const profile_member_declaration& member : record.members) {
      ...
      if (member.subject().kind() == requirement_subject_kind::package) {
        ...
        const computed_profile& nested = compute(member.subject().profile().name());
        ...
        for (const profile_expansion_path& nested_path : nested.expansion) {
          ...
          steps.insert(steps.end(), nested_path.steps().begin(),
    ...
    auto inserted = computed_.emplace(
        name, computed_profile{
                  profile_identity::from_sha256(writer.finish()),
                  std::move(expansion)});
                             ...
                             std::map<std::string, sealed_profile>& closure)
{
  ...
  for (const profile_member_declaration& member : profile.direct_members())
    if (member.subject().kind() == requirement_subject_kind::profile)
      collect_profile_closure(catalog.require(member.subject().profile()),
                              catalog, closure);
}
...
profile_member_declaration::provenance() const noexcept { return provenance_; }

profile_declaration::profile_declaration(
    profile_reference name, declaration_provenance provenance,
    std::vector<profile_member_declaration> members)
...
const profile_reference& profile_declaration::name() const noexcept { return name_; }
const declaration_provenance& profile_declaration::provenance() const noexcept
{
  return provenance_;
}
const std::vector<profile_member_declaration>&
profile_declaration::members() const noexcept { return members_; }

profile_expansion_step::profile_expansion_step(
    profile_reference profile, requirement_subject member,
    declaration_provenance provenance)
...
const declaration_provenance& profile_expansion_step::provenance() const noexcept
{
  return provenance_;
}
...
profile_expansion_path::steps() const noexcept { return steps_; }
...
sealed_profile::sealed_profile(
    profile_reference name, profile_identity identity,
    declaration_provenance provenance,
...
const profile_reference& sealed_profile::name() const noexcept { return name_; }
const profile_identity& sealed_profile::identity() const noexcept { return identity_; }
const declaration_provenance& sealed_profile::provenance() const noexcept
{
  return provenance_;
}
const std::vector<profile_member_declaration>&
sealed_profile::direct_members() const noexcept { return direct_members_; }
const std::vector<profile_expansion_path>&
sealed_profile::expansion() const noexcept { return expansion_; }

profile_catalog::profile_catalog(std::vector<sealed_profile> profiles)
    : profiles_(std::move(profiles))
{
}
profile_catalog profile_catalog::seal(
...
const sealed_profile& profile_catalog::require(
    const profile_reference& profile) const
{
  const auto found = std::lower_bound(
      profiles_.begin(), profiles_.end(), profile,
...
const std::vector<profile_expansion_step>&
requirement_origin::expansion() const noexcept { return expansion_; }
...
resolved_requirement::resolved_requirement(
    requirement_scope scope, package_reference package,
...
resolved_requirement::origins() const noexcept { return origins_; }
...
selected_profile::selected_profile(
    profile_reference profile, profile_identity identity,
...
selected_profile::declarations() const noexcept { return declarations_; }
...
sealed_requirement_set sealed_requirement_set::seal(
    ...
    const profile_catalog& profiles)
{
  ...
  for (const requirement_declaration& declaration : declarations) {
    ...
    const sealed_profile& profile = profiles.require(declaration.subject().profile());
  ...
  for (auto& entry : requirements)
    resolved.emplace_back(entry.first.first, entry.first.second,
  ...
  for (auto& entry : selected) {
    ...
    selected_profiles.emplace_back(profile.name(), profile.identity(),
                                   std::move(entry.second));
  }
...
sealed_requirement_set::requirements() const noexcept { return requirements_; }
std::vector<resolved_requirement> sealed_requirement_set::for_scope(
